...
public class ReservationController : ControllerBase
{
    ...
    private bool IsStaff => User.IsInRole("Backoffice") || User.IsInRole("GridOperator") || User.IsInRole("Admin") || User.IsInRole("MicrogridOperator");
    ...
    {
        var staff = IsStaff; var nic = staff ? null : await CurrentNicAsync();
        if (!staff && string.IsNullOrWhiteSpace(nic)) return Unauthorized(ApiResponse<object>.FailureResponse("The prosumer profile has no NIC. Contact Backoffice to complete the profile."));
        var rows = await _service.GetAsync(nic, status, nodeId, Math.Max(1,page), Math.Clamp(pageSize,1,100));
    ...
    {
        var targetNic = IsStaff ? nic : await CurrentNicAsync();
        if (!IsStaff && string.IsNullOrWhiteSpace(targetNic)) return Unauthorized(ApiResponse<object>.FailureResponse("The prosumer profile has no NIC."));
        return Ok(ApiResponse<object>.SuccessResponse(await _service.SummaryAsync(targetNic?.Trim().ToUpperInvariant())));
    }
    [HttpGet("{id}")]
    public async Task<IActionResult> GetById(string id)
    {
        var row = await _service.GetByIdAsync(id); if (row == null) return NotFound(ApiResponse<object>.FailureResponse("Reservation not found."));
        if (!IsStaff && row.ProsumerId != await CurrentNicAsync()) return StatusCode(403, ApiResponse<object>.FailureResponse("You can only view your own reservations."));
        return Ok(ApiResponse<object>.SuccessResponse(row));
    }
    [HttpPost]
    public async Task<IActionResult> Create([FromBody] CreateReservationDto dto)
    {
        try { var row = await _service.CreateAsync(dto, ActorId, IsStaff); return CreatedAtAction(nameof(GetById), new { id = row.Id }, ApiResponse<object>.SuccessResponse(row, "Reservation created and awaiting approval.")); }
    ...
    {
        try { return Ok(ApiResponse<object>.SuccessResponse(await _service.UpdateAsync(id, dto.EnergySlotId, dto.EnergyAmount, ActorId, IsStaff), "Reservation updated.")); }
    ...
    {
        try { return Ok(ApiResponse<object>.SuccessResponse(await _service.MarkCompletedAsync(id, ActorId), "Reservation completed.")); }
        catch (KeyNotFoundException ex) { return NotFound(ApiResponse<object>.FailureResponse(ex.Message)); }
        catch (ArgumentException ex) { return BadRequest(ApiResponse<object>.FailureResponse(ex.Message)); }
    }
    ...
    {
        try { return Ok(ApiResponse<object>.SuccessResponse(await _service.TransitionAsync(id, action, ActorId, IsStaff, reason), $"Reservation {action}d.")); }
        catch (KeyNotFoundException ex) { return NotFound(ApiResponse<object>.FailureResponse(ex.Message)); }
        catch (UnauthorizedAccessException ex) { return StatusCode(403, ApiResponse<object>.FailureResponse(ex.Message)); }
        catch (ArgumentException ex) { return BadRequest(ApiResponse<object>.FailureResponse(ex.Message)); }
        catch (InvalidOperationException ex) { return Conflict(ApiResponse<object>.FailureResponse(ex.Message)); }
