    ...
    public static class DbSeeder
    {
        ...
        public static async Task SeedDefaultUsersAsync(MongoDbContext context)
        {
            ...
            if (userCount == 0)
            {
                ...
                // ──────────────────────────────────────────────
                var mg1  = MakeGrid("Colombo Solar Hub",          "Colombo",        "High-efficiency 500kW commercial solar array in Central Colombo.",                     6.9271,  79.8612, 500, 350, 100,  50, 200, 160, 80, 24, "Active",      true,  opAlex,   now);
                var mg2  = MakeGrid("Kandy Highland Microgrid",   "Kandy",          "Highland solar and battery storage microgrid serving local tea estates.",               7.2906,  80.6337, 300, 200,  50,  50, 150, 105, 70, 18, "Active",      true,  opKavindu,now);
                var mg3  = MakeGrid("Galle Coastal Solar Park",   "Galle",          "Coastal solar farm with lithium storage backup.",                                      6.0535,  80.2210, 450,   0,   0,   0, 180,  45, 25, 20, "Maintenance", false, opLakshmi,now);
                var mg4  = MakeGrid("Jaffna Northern Grid",       "Jaffna",         "Northern peninsula solar micro-grid feeding rural electrification.",                    9.6615,  80.0255, 350, 280,  40,  30, 120,  96, 80, 16, "Active",      true,  opAlex,   now);
                var mg5  = MakeGrid("Negombo Lagoon Solar Farm",  "Negombo",        "Floating solar panels on Negombo lagoon with smart inverter tech.",                     7.2083,  79.8358, 600, 450, 100,  50, 250, 200, 80, 32, "Active",      true,  opKavindu,now);
                var mg6  = MakeGrid("Nuwara Eliya Green Energy",  "Nuwara Eliya",   "High-altitude hybrid wind and solar micro-grid.",                                      6.9497,  80.7891, 200, 140,  30,  30, 100,  60, 60, 12, "Active",      true,  opLakshmi,now);
                var mg7  = MakeGrid("Trincomalee Eastern Hub",    "Trincomalee",    "Eastern coastal solar installation with EV charging integration.",                      8.5874,  81.2152, 400, 300,  60,  40, 160, 128, 80, 20, "Active",      true,  opAlex,   now);
                var mg8  = MakeGrid("Matara Southern Solar",      "Matara",         "Southern district distributed solar panels across multiple rooftops.",                  5.9485,  80.5353, 250, 180,  40,  30, 100,  50, 50, 14, "Active",      true,  opKavindu,now);
                var mg9  = MakeGrid("Batticaloa Solar Station",   "Batticaloa",     "Arid zone concentrated solar power station.",                                          7.7310,  81.6747, 380, 300,  50,  30, 140,  98, 70, 22, "Active",      true,  opLakshmi,now);
                var mg10 = MakeGrid("Anuradhapura Heritage Grid", "Anuradhapura",   "Ancient city solar-powered heritage preservation micro-grid.",                          8.3114,  80.4037, 280,   0,   0,   0, 110,  22, 20, 14, "Offline",     false, opAlex,   now);
                ...
                var reservations = new List<Reservation>
                {
                    ...
                    MakeReservation(nicRuwan,    mg4.Id!, slots[4].Id!, 12.0, 2,  6, "Completed", now, nicRuwan, opAlex,  opAlex),
                    ...
                    MakeReservation(nicAmaya,    mg6.Id!, slots[7].Id!, 5.0,  3,  7, "Completed", now, nicAmaya, opLakshmi, opLakshmi),
            ...
            string status, bool isActive, string operatorId, DateTime now)
        {
            return new MicrogridNode
            {
                ...
                BatteryPercentage = batPct, BatteryStorageSlots = batteryStorageSlots,
